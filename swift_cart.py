...
Delivery_time_category=df.groupby("Delivery_time_category")['is_returning'].size().reset_index()

Delivery_time_category.columns=["Delivery_time","Returning_customers"]
...
daily_city = df.groupby(["city","order_date"])["order_value"].sum().reset_index()
